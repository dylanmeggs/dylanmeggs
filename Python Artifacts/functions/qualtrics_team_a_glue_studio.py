...
input_bucket_name = 'qualtrics-team-raw-data'
output_bucket_name = 'aws-data-lake-prod'
input_directory = 'input-files'
output_directory = 'qualtrics_team_prod'
...
#objects = s3.list_objects_v2(Bucket=input_bucket_name, Prefix=input_directory)
#latest_csv = sorted([obj['Key'] for obj in objects['Contents'] if obj['Key'].endswith('.csv')], key=lambda x: x['LastModified'], reverse=True)[0]
...
df.to_csv(f's3://{output_bucket_name}/{output_directory}/{output_file_name}', index=False)